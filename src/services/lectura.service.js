...
async function runLectura(pool, id) {
  ...
  // --- DEBUG (añade estas dos líneas) ---
console.log("AUTO_FASE2(raw) =", JSON.stringify(process.env.AUTO_FASE2));
const autoF2 = (process.env.AUTO_FASE2 || "").toLowerCase().trim();

  // 7) ⬇️ encadenar fase 2 (ElevenLabs) automáticamente si AUTO_FASE2=1
  try {
    const autoF2 = (process.env.AUTO_FASE2 || "").toLowerCase().trim();
if (["1", "true", "on", "yes"].includes(autoF2)) {
